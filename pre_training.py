...
class Pretrain(Dataset):
    def __init__(self, tokenizer, type_path, num_samples, input_length, output_length, print_text=False):
      self.dataset = self.split_into_segment(pd.read_csv(prefix_path+"dataset.csv"),input_length)
    ...
    def split_into_segment(self, ds, input_length):
        ...
        new_r=[]
        for p in range(len(new_rows)):
          if len(new_rows[p].split())>5:
            new_r.append(new_rows[p])
        ds2 = pd.DataFrame(new_r, columns=['context'])
        ds = ds.append(ds2)
        return ds
    ...
    def __getitem__(self, index):
        #print("dataset",self.dataset.iloc[index])
